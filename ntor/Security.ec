       ...
       (tested = None <=> tq = None)
        ...
        /\ (forall x y sk b, tq = Some (g ^ (ZModE.( * ) x y), g ^ (ZModE.( * ) x sk), b, g ^ x, g ^ y) 
                => (g ^ (ZModE.( * ) x y), g ^ (ZModE.( * ) x sk), b, g ^ x, g ^ y) \in h2m 
                         /\ ((exists i pk_s sk_ce tag key ir, csm.[i] = Some (Accepted (pk_s, sk_ce) ((pk_s, g ^ x), Some (g ^ y, tag)) key ir)
                     /\ get_ir_test (oget csm.[i]))
                  \/ (exists i sk_s sk_se tag key ir, ssm.[i] = Some (Accepted (sk_s, Some sk_se) ((g ^ sk_s, g ^ x), Some (g ^ y, tag)) key ir)
        ...
        /\ (forall x y sk b, (g ^ (ZModE.( * ) x y), g ^ (ZModE.( * ) x sk), b, g ^ x, g ^ y) \in h2m
                => (exists i pk_s sk_ce tag key ir, csm.[i] = Some (Accepted (pk_s, sk_ce)
                                  ((pk_s, g ^ x), Some (g ^ y, tag)) key ir)
                     /\ (get_ir_sess (oget csm.[i]) \/ get_ir_test (oget csm.[i])))
                  \/ (exists i sk_s sk_se tag key ir, ssm.[i] = Some (Accepted (sk_s, Some sk_se)
                                  ((g ^ sk_s, g ^ x), Some (g ^ y, tag)) key ir)
...
call (: Game4.badq
      , ={servers, c_smap, s_smap, tested, kp_set, hm, bad, h1m, hq, tq, badq, tags, badt}(Game4, Game4)
        ...
        /\ (inv_Game4 Game4.tested Game4.tq Game4.badq Game4.kp_set Game4.s_smap Game4.hq Game4.c_smap Game4.h1m Game4.h2m Game4.tags Game4.servers){2}
        /\ (forall x, x \in Game4.h2m{1} <=> x \in Game4.h2m{2})
      , ={badq}(Game4, Game4)) => //; last first.

- auto => />.
split; 1: by smt(emptyE in_fset0).
move => ntc nts ninkps injc pkins injs trs pc acc acs inv acas tgas skpk rl rr al bl bql csl hql kpl ssl sl tl tql h1ml h2ml btl tgsl ar br bqr csr hqr kpr ssr sr tr tqr h1mr h2mr btr tgsr. 
by case : (!bqr) => />. 
...
- proc; inline.
  ...
  if => //.
  + auto => /> &1 &2 kps ? ? _ ? _ ? ? ? c1 ? inv5 ? ? ? ? ? ? inv ? inv2 ? inv6 inv3 inv4 inv7 *.
    split; 1: by smt(get_setE in_fsetU1).
    split. 
    + move => x0 y0 sk0 b0 tqeq.
...
by smt(get_setE).
    ...
    smt(get_set_neqE).
  auto => /> &1 &2 kps ? ? _ ? _ ? ? ? c1 ? inv5 ? ? ? ? ? ? inv2 ? ? ? inv inv3 inv4 inv7*.
  split; 1: by smt(get_setE in_fsetU1).
  split. 
  + move => x0 y0 sk0 b0 tqeq.
    have := inv5 x0 y0 sk0 b0 tqeq.
...
- proc; inline.
  ...
  + auto => /> &1 &2 ? ? ? ? ? ? ? ? inv2 ? ? _ ? ? inv4 ? ? ? ? inv5 inv inv3 *.
    ...
    have:= inv3 (m3{2}.`1 ^ sk_ce{2}, b{2} ^ sk_ce{2}, b{2}, g ^ sk_ce{2}, m3{2}.`1).
    have->: (m3{2}.`1 ^ sk_ce{2}, b{2} ^ sk_ce{2}, b{2}, g ^ sk_ce{2}, m3{2}.`1) \in Game4.tags{2} by smt().
    simplify. 
    move => [H1] [H2] [j st'' k'' ir''] H3 H4.
    exists j st'' k'' ir''.
    have->: (t0 = ((b{2}, g ^ sk_ce{2}), Some (m3{2}.`1, oget Game4.tags{2}.[m3{2}.`1 ^ sk_ce{2}, b{2} ^ sk_ce{2}, b{2}, g ^ sk_ce{2}, m3{2}.`1]))).
    + have->: (t0 = ((b{2}, g ^ sk_ce{2}), Some m3{2})) by smt(get_setE).
  ...
  + auto => /> &1 &2 ? ? ? ? ? ? ? inv2 ? ? ? inv3 ? ? ? ? ? ? inv *.
    ...
    + split; 1: by smt().
      ...
      + move => x0 y0 sk0 b0 x0in.
        ...
        + left.
          exists i{2} st'.`1 st'.`2 (oget t'.`2).`2 k' (ir'.`1, true, ir'.`3).
          smt(get_setE).
...
(************ case that the handle was in h2m ************)
  ...
  + case (Game4.tested{2} <> None) => test; 2: by smt().
    ...
    move => eq. rewrite eq.
    move => tqeq2.
    have := inv2 st'.`2 (loge (oget t'.`2).`1) (loge st'.`1) st'.`1 tqeq2.
    ...
    + rewrite /untested_partner_c.
      ...
      + rewrite /get_partners_c.
        ...
        split. 
        + move => [x0in trx0]. 
          apply (inv3 x0 i0 t'.`1 (oget t'.`2).`1 (oget t'.`2).`2 (g ^ sk_s, g ^ st'.`2) t); smt().
        move => ->.
        split; 1: by smt().
        rewrite H1 /get_trace //=.
        have->: (g ^ st'.`2 = (t'.`1).`2). smt().
        have->: g ^ sk_s = (t'.`1).`1. admit.
        have->: t = (oget t'.`2).`2.
        + have := inv4 i{2} st' t' k' ir' stc.
          move => [j st'' k'' ir''].
          have := inv3 i0 j (g ^ sk_s, g ^ st'.`2) (oget t'.`2).`1  t t'.`1 (oget t'.`2).`2. smt().
      ...
      + rewrite /get_untested_partners_c.
        ...
        have:= inv3 x0 i0 t'.`1 (oget t'.`2).`1 (oget t'.`2).`2 (g ^ sk_s, g ^ st'.`2) t.
        smt().
      smt(fcard1 fcards0).
    smt().
  split; 1: by smt().
  ...
  + auto => /> &1 &2 ? ? ? ? ? c1 c2 inv2 c3 c4 _ ? ? ? ? ? ? ? inv inv3 inv4 *.
    ...
    + split; 1: by smt().
      ...
      + move => x0 y0 sk0 b0 x0in.
        ...
        + right. left.
          exists (b, j){2} st'.`1 (oget st'.`2) (oget t'.`2).`2 k' (ir'.`1, true, ir'.`3).
          smt(get_setE).
        have := inv x0 y0 sk0 b0.
...
(************ case that the handle was in h2m ************)
  ...
  + case (Game4.tested{2} <> None) => test; 2: by smt().
    ...
    move => eq. rewrite eq.
    move => tqeq2.
    have := inv2 (loge (t'.`1).`2) (oget st'.`2) st'.`1 (g ^ st'.`1) tqeq2.
    rewrite //= some_oget //=; 1: smt().
    move => [] tqin [[i0 pk_s sk_ce t k ir [H1 H2]]|[i0 sk_s sk_se t k ir [H1 H2]]]. 
      ...
      + rewrite /untested_partner_s.
        ...
        + rewrite /get_partners_s.
          ...
          + move => [x0in trx0].
            ...
            admit. (* I need somehow that (pk_s = (t'.`1).`1) *)
          move => ->.
          split; 1: by smt().
          rewrite H1 /get_trace //=.
          have->: (g ^ loge (t'.`1).`2 = (t'.`1).`2). by rewrite expgK.
          have->: t = (oget t'.`2).`2.
          + have := inv4 i0 (pk_s, sk_ce) ((pk_s, g ^ loge (t'.`1).`2), Some (g ^ oget st'.`2, t)) k ir H1.
            move => [i st'' k'' ir''] H3.
            have := inv5 (b,j){2} i t'.`1 (oget t'.`2).`1 (oget t'.`2).`2 t'.`1 t.
            move => H4.
            have : (b,j){2} = i by admit.
            move => ieq.
  ...
  + move => x0 y0 sk0 b0 x0in.
    ...
    + right; left. 
      ...
      exists i' sk_s' sk_se t k ir. 
      have : get_trace (oget (Some (Accepted st' t' k' (ir'.`1, true, ir'.`3)))) = get_trace (oget Game4.s_smap{2}.[b{2}, j{2}]); 1: by smt().
      case (i' = (b,j){2}); 2: smt(get_setE mem_set). admit.
    by smt().
  split.
  + move => i st0 t k0 ir H1. 
...
- proc; inline.
  ...
  match = => // sk.
  if => //.
  auto => /> &1 &2 *. split. smt(get_setE mem_set). 
  move => b0 sk0.
  case (b0 = b{2}) => beq.
  admit. 
  + smt(get_setE).
...
- proc; inline.
  ...
  + rcondt {1} ^if. auto => /#.
    ...
    + split.
      + move => x0 y0 sk0 eq1 eq2 eq3 eq4.
        split. smt(mem_set).
        left.
        exists i{2} st'.`1 st'.`2 (oget t'.`2).`2 k' (ir'.`1, ir'.`2, true).
        smt(get_setE).
      split; 1: by smt(get_setE).
      split; 1: by smt(get_setE).
      split; 1: by smt(get_setE).
      ...
      split.
      + move => x0 y0 sk0 b0 x0in.
        case ((g ^ (ZModE.( * ) x0 y0), g ^ (ZModE.( * ) x0 sk0), b0, g ^ x0, g ^ y0) = ((oget t'.`2).`1 ^ st'.`2, st'.`1 ^ st'.`2, st'.`1, g ^ st'.`2, (oget t'.`2).`1)) => [[] in1 in2 in3 in4 in5|neq].
        + left.
          exists i{2} st'.`1 st'.`2 (oget t'.`2).`2 k' (ir'.`1, ir'.`2, true).
          smt(get_setE).
        have := inv x0 y0 sk0 b0.
  ...
  suff //=:false.
  have := inv st'.`2 (loge (oget t'.`2).`1) (loge st'.`1) st'.`1.
  have->: (g ^ (st'.`2 * loge (oget t'.`2).`1), g ^ (st'.`2 * loge st'.`1), st'.`1, g ^ st'.`2, g ^ loge (oget t'.`2).`1) = ((oget t'.`2).`1 ^ st'.`2, st'.`1 ^ st'.`2, st'.`1, g ^ st'.`2, (oget t'.`2).`1). 
  congr.
  + by rewrite ComRing.mulrC expM expgK.
  + by rewrite ComRing.mulrC expM expgK.
  by rewrite expgK. 
  ...
  + rewrite negb_exists. 
    ...
    move => ir.
    rewrite negb_and.
    case (Game4.c_smap{2}.[int] = Some (Accepted (pk_s, sk_ce) ((pk_s, g ^ st'.`2), Some (g ^ loge (oget t'.`2).`1, t)) k ir)); 2: by done.
    simplify. 
    move => stint.
    have := inv2 int i{2} (pk_s, g ^ st'.`2) (Some (g ^ loge (oget t'.`2).`1, t)) t'.`2.
    admit.
  split.
  + rewrite negb_exists. 
    move => int.
    ...
    move => ir.
    rewrite negb_and.
    case (Game4.s_smap{2}.[int] = Some (Accepted (sk_s, Some sk_se) ((g ^ sk_s, g ^ st'.`2), Some (g ^ loge (oget t'.`2).`1, t)) k ir)); 2: by done.
    simplify.
    move => stint.
    rewrite negb_or.
    case (ir.`2) => rev.
    + have : fresh_partner_c t' Game4.s_smap{2} Game4.servers{2} = Some false; 2: by smt().
      ...
      case (get_trace (oget Game4.s_smap{2}.[x0]) = Some t'); 2: by smt().
      have:= (inv4 x0 int t'.`1 (g ^ loge (oget t'.`2).`1) (oget t'.`2).`2 (g ^ sk_s, g ^ st'.`2) t). 
      by smt(expgK).
...
- proc; inline.
  ...
  + rcondt {1} ^if. auto => /#.
    ...
    + split.
      + move => x0 y0 sk0 eq1 eq2 eq3 eq4.
        split; 1: by  smt(mem_set).
        right.
        exists (b,j){2} st'.`1 (oget st'.`2) (oget t'.`2).`2 k' (ir'.`1, ir'.`2, true).
        smt(get_setE).
      split.
      + move => // i0 i'.
        case (i0 = (b, j){2}) => ieq.
      ...
      split.
      + move => x0 y0 sk0 b0 x0in.
        case ((g ^ (ZModE.( * ) x0 y0), g ^ (ZModE.( * ) x0 sk0), b0, g ^ x0, g ^ y0) = ((t'.`1).`2 ^ oget st'.`2, (t'.`1).`2 ^ st'.`1, g ^ st'.`1, (t'.`1).`2, (oget t'.`2).`1)) => [[] in1 in2 in3 in4 in5|neq].
        + right; left.
          exists (b,j){2} st'.`1 (oget st'.`2) (oget t'.`2).`2 k' (ir'.`1, ir'.`2, true).
          smt(get_setE).
        have := inv x0 y0 sk0 b0.
  ...
  suff //=:false.
  have := inv (loge (t'.`1).`2) (oget st'.`2) st'.`1 (g ^ st'.`1).
  ...
  + rewrite negb_exists. 
    ...
    move => ir.
    rewrite negb_and.
    case (Game4.c_smap{2}.[int] = Some (Accepted (pk_s, sk_ce) ((pk_s, g ^ loge (t'.`1).`2), Some (g ^ oget st'.`2, t)) k ir)); 2: by done.
    simplify. 
    move => stint.
    rewrite negb_or.
    case (ir.`2) => rev.
    + have : fresh_partner_s t' Game4.c_smap{2} = Some false; 2: by smt().
      ...
      + rewrite /get_origins_s.
        ...
        have->: t'.`1 = (pk_s, g ^ loge t'.`1.`2). admit.
        smt(get_setE).
      move => H3.
      ...
      have:= (inv3 int x0 (pk_s, g ^ loge (t'.`1).`2) (Some (g ^ oget st'.`2, t))). 
      admit.
    case (ir.`3) => tes; 2: by smt().
    by smt().
  split.
  + rewrite //=.
    ...
    move => ir.
    rewrite negb_and.
    case (Game4.s_smap{2}.[int] = Some (Accepted (sk_s, Some sk_se) ((g ^ sk_s, g ^ loge (t'.`1).`2), Some (g ^ oget st'.`2,  t)) k ir)); 2: by done.
    simplify.
    move => stint.
    have : int = (b,j){2}; 1: by have := inv2 int (b,j){2} (g ^ sk_s, g ^ loge (t'.`1).`2) (oget t'.`2).`1 t t'.`1 (oget t'.`2).`2; smt().
    move => inteq.
    rewrite negb_or.
    split. 
...
lemma tested_nn &m: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested = None] = 0%r.
proof.
byphoare => //; hoare.
proc; inline.
call (_: ! (Game4.badq /\ Game4.tested = None) /\ (Game4.tested = None <=> Game4.tq = None)); 2..10: conseq (: true) => //.
...
lemma test_i_pr &m: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq] = Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ (exists i, Game4.tested = Some i)].
proof.
have->: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq] = Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested = None] 
               + Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested <> None] by rewrite Pr[mu_split Game4.tested = None].
have->: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested <> None] = Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ (exists i, Game4.tested = Some i)].
+ rewrite Pr[mu_eq] // => &hr.
  by smt().
by smt(tested_nn).
qed.
...
lemma sum_pr &m: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ (exists i, Game4.tested = Some i)] = 
                   big predT (fun i => Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested = Some i]) (range 1 (max_qc + 1))
                   + Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ (exists i, Game4.tested = Some i) /\ !mem (range 1 (max_qc + 1)) (oget Game4.tested)].
proof.
...
+ elim: (range 1 (max_qc + 1)) (range_uniq 1 (max_qc + 1)) => /=; 1: by rewrite big_nil Pr[mu_false].
  ...
  have ->: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : ((((Game4.badq /\ exists (i : int), Game4.tested = Some i) /\ Game4.badq) /\
     exists (i : int), Game4.tested = Some i) /\ oget Game4.tested = x) /\ (oget Game4.tested \in xs)]
         = Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : false].
  + rewrite Pr[mu_eq] // => &hr.
    by smt().
...
lemma test_ephrev_nn &m i: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : (Game4.badq /\ Game4.tested = Some i) /\ Game4.test_ephrev_s = None] = 0%r.
proof.
byphoare => //; hoare.
proc; inline.
call (_: ! (Game4.badq /\ Game4.tested = None /\ Game4.test_ephrev_s = None) /\ (Game4.test_ephrev_s = None <=> Game4.tq = None) /\ (Game4.tested = None <=> Game4.tq = None)); 2..10: conseq (: true) => //.
...
lemma split_pr &m i: Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested = Some i] = 
                     Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested = Some i /\ Game4.test_ephrev_s = Some true] 
                     + Pr[E_GAKE_nodhs(Game4, A).run(false) @ &m : Game4.badq /\ Game4.tested = Some i /\ Game4.test_ephrev_s = Some false].
proof.
...
call (: ={b0, hm, servers, c_smap, s_smap, tested, kp_set, bad, hq, tq, badq, tags, badt, counti, handles_c, test_ephrev_s, b_test, j_test}(Game4, GameDDH)
        ...
        /\ (forall x, Game4.h2m{1}.[x] = GameDDH.h2mDDH{2}.[(clear_ddh x)])
 (*       /\ (forall x, x \in Game4.h2m{1} => x \in Game4.h1m{1})*)); 2,3, 8, 9,10: sim />; last first.
